...
#' @param genotypeData A dataframe containing genotypic data if genotypeFile not provided
...
#' )
#' }
runGPCP = function(phenotypeFile, genotypeFile=NA, genotypeData=NA, genotypes, traits,
  ...
  if (length(userFixed) == 1L && !is.na(userFixed) && nzchar(userFixed)) {
       # a single comma‐sep string: split it
        userFixed <- strsplit(userFixed, ",", fixed = TRUE)[[1]]
  } else if (length(userFixed) > 1L) {
        # already a character vector: leave as‐is
        userFixed <- as.character(userFixed)
  } else {
  ...
  if (!(length(genotypeData) == 1L && is.na(genotypeData))) {
        G <- genotypeData
  }
  ...
  for (i in 1:length(userResponse)) {
    ...
    myMod <- tryCatch(
      ...
      error = function(e) {
        message("Singular V matrix: retrying with tolParInv = 1e-1")
        sommer::mmes(
          fixed    = stats::as.formula(fixedArg),
          random   = stats::as.formula(randArg),
          rcov     = ~units,
  ...
  for (i in 1:length(userModels)) {
    ...
    # backsolve
    addEff <- A.TTinv %*% matrix(subModA, ncol = 1)
    domEff <- D.TTinv %*% matrix(subModD, ncol = 1)
